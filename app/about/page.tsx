  ...
  return (
    <>
      ...
      <section className="section-shell section-block about-section ruled-section">
        <div><p className="section-kicker">Background</p><h2>The way I work</h2></div>
        <div className="body-stack">{profile.biography.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div>
      </section>
      <section className="section-shell section-block about-section ruled-section">
        <div><p className="section-kicker">Toolkit</p><h2>Tools I work with</h2></div>
        <ul className="skill-list">{profile.tools.map((tool) => <li key={tool}>{tool}</li>)}</ul>
      </section>
      <section className="section-shell section-block about-section ruled-section">
        <div><p className="section-kicker">Experience</p><h2>Work, research &amp; teaching</h2></div>
      ...
      {profile.education?.length ? <section className="section-shell section-block about-section ruled-section">
        <div><p className="section-kicker">Education</p><h2>What I&apos;m studying</h2></div>
        <div className="timeline">{profile.education.map((item) => (
          <article className="timeline-item" key={item.institution}><p className="entry-meta">{item.dates}</p><h3>{item.qualification}</h3><p className="timeline-meta">{item.institution}</p><p>{item.detail}</p></article>
        ))}</div>
      </section> : null}
      {profile.languages?.length ? <section className="section-shell section-block about-section ruled-section">
        <div><p className="section-kicker">Communication</p><h2>Languages</h2></div>
        <div><ul className="skill-list">{profile.languages.map((language) => <li key={language}>{language}</li>)}</ul><p className="qualification-note">{profile.qualifications?.join(" / ")}</p></div>
      </section> : null}
      <section className="writing-band">
        <div className="section-shell section-block about-section">
          <div><p className="section-kicker">Off the clock</p><h2>Other interests</h2></div>
          <div><ul className="interest-list">{profile.interests.map((interest) => <li key={interest}>{interest}</li>)}</ul><div className="inline-links"><Link className="text-link" href="/writing">Writing <ArrowUpRight size={17} aria-hidden="true" /></Link><Link className="text-link" href="/photos">Photos <ArrowUpRight size={17} aria-hidden="true" /></Link></div></div>
        </div>
      </section>
    </>
  );
